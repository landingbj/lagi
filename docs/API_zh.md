...
检索增强`rag`功能需要用到向量数据库，您可以根据您的喜好选择您喜欢的数据库。

***以chromadb为例：***

您可以直接通过docker运行以下命令,拉取安装并启动chromadb。
...
```
您可以通过浏览器访问：http://localhost:8000/docs 查看是否启动成功。

### 2.配置yml文件

修改[`src/main/resources/lagi.yml`]() 配置文件，选择您喜欢的模型，将其中的大语言模型your-api-key等信息替换为您自己的密钥，并根据需要将启用的模型的`enable`字段设置为`true`。详细的配置方法可参见[配置文档](docs/config_zh.md)。
