...
当你需要对话返回的敏感词进行过滤时, 你可以往 sensitive_word.json 文件里添加你所希望过滤的词语. 如：
...
当你需要对期望模型优先给出和某些词语有关的回答时， 你可以在 priority_word.json 文件里添加相应的词语， 应用会优先通过文件里的词语，构建相应的提示词, 从而影响模型的响应。如：
...
你可以通过在 stopping_word.json 里添加停止词来让应用知道你希望对话从哪里停止， 这样用户再进行对话时， 应用会根据新的对话构建提示词。 如：
```json
[
  "stop"
]
