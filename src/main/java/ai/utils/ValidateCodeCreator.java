...
import ai.migrate.pojo.ValidateCode;
...
public class ValidateCodeCreator {
	...
	public static String[] defaultChars = { "1", "2", "3", "4", "5", "6", "7", "8", "9", "$", "#", "a", "b", "c", "d", "e", "g", "h", "j", "k", "m", "n", "p", "q", "s", "u", "v", "w", "x", "y", "z" };
	...
	public static ValidateCode create(int width, int height, int charNum, String[] chars, int fontSize) throws IOException {
		if (chars == null) {
			chars = defaultChars;
		}
		...
		for (int i = 0; i < charNum; i++) {
			String rand = chars[random.nextInt(len)];
			sRand += rand;
			...
			g.setFont(ft);
			g.drawString(rand, unitWidth * i + fontOffset, fontSize);
		}

		g.dispose();
		ValidateCode validateCode = new ValidateCode();
		validateCode.setImage(image);
		validateCode.setRand(sRand);
