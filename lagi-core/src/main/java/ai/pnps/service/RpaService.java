...
//        params.put("username", "女儿红");
    ...
    public boolean startRobot(StartRobotRequest request) {
        ...
        for (String appId : request.getAppIdList()) {
            SocialPnpParam param = SocialPnpParam.builder()
                    ...
                    .robotFlag(robotEnable ? PnpGlobal.ENABLE_FLAG : PnpGlobal.DISABLE_FLAG)
                    .timerFlag(PnpGlobal.ENABLE_FLAG)
    ...
    public static class WorkerThread extends Thread {
        ...
        private final SocialPnpParam param;

        public WorkerThread(SocialPnpParam param, long sleepTime) {
        ...
        @Override
        public void run() {
            SocialPnp pnp = SocialPnpFactory.getSocialPnp(param);
            Worker<Boolean, Boolean> worker = new RobotWorker(pnp);
            worker.notify(true);
            try {
