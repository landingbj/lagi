...
public class McpAsyncServer {
	 ...
	 * Specification</a>
	 * @deprecated This will be removed in 0.9.0. Use
	 * {@link McpAsyncServerExchange#createMessage(CreateMessageRequest)}.
	 */
	@Deprecated
	...
	private static class AsyncServerImpl extends McpAsyncServer {
		...
		private McpServerSession.RequestHandler<ListResourceTemplatesResult> resourceTemplateListRequestHandler() {
			return (exchange, params) -> Mono
				.just(new ListResourceTemplatesResult(this.resourceTemplates, null));
	...
	private static final class LegacyAsyncServer extends McpAsyncServer {
		...
		LegacyAsyncServer(ServerMcpTransport mcpTransport, McpServerFeatures.Async features) {
			...
			List<BiFunction<McpAsyncServerExchange, List<Root>, Mono<Void>>> rootsChangeHandlers = features
				.getRootsChangeConsumers();

			List<Function<List<Root>, Mono<Void>>> rootsChangeConsumers = rootsChangeHandlers.stream()
				.map(handler -> (Function<List<Root>, Mono<Void>>) (roots) -> handler.apply(null, roots))
				.collect(Collectors.toList());
		...
		// Lifecycle Management
		// ---------------------------------------
		private McpClientSession.RequestHandler<InitializeResult> asyncInitializeRequestHandler() {
			return params -> {
		 ...
		 */
		public Mono<ListRootsResult> listRoots(String cursor) {
			return this.mcpSession.sendRequest(McpSchema.METHOD_ROOTS_LIST, new PaginatedRequest(cursor),
					LIST_ROOTS_RESULT_TYPE_REF);
		}

		private McpClientSession.NotificationHandler asyncRootsListChangedNotificationHandler(
				List<Function<List<Root>, Mono<Void>>> rootsChangeConsumers) {
		...
		private McpClientSession.RequestHandler<ListToolsResult> toolsListRequestHandler() {
			return params -> {
				List<Tool> tools = this.tools.stream().map(McpServerFeatures.AsyncToolSpecification::getTool).collect(Collectors.toList());
		...
		private McpClientSession.RequestHandler<ListResourcesResult> resourcesListRequestHandler() {
			return params -> {
				var resourceList = this.resources.values()
						.stream()
						.map(McpServerFeatures.AsyncResourceSpecification::getResource)
		...
		private McpClientSession.RequestHandler<ListResourceTemplatesResult> resourceTemplateListRequestHandler() {
			return params -> Mono.just(new ListResourceTemplatesResult(this.resourceTemplates, null));

		}

		private McpClientSession.RequestHandler<ReadResourceResult> resourcesReadRequestHandler() {
			return params -> {
		...
		private McpClientSession.RequestHandler<ListPromptsResult> promptsListRequestHandler() {
			return params -> {
				// TODO: Implement pagination
		...
		private McpClientSession.RequestHandler<GetPromptResult> promptsGetRequestHandler() {
			return params -> {
